  ...
  <div class="schedules-view page">
    <div class="page-header">
      <div class="header-content">
        ...
        <p class="page-subtitle">Configure recurring tasks like backups and restarts.</p>
      </div>
    ...
    <el-card v-loading="loading">
      <el-table :data="schedules" stripe style="width: 100%">
        ...
        <el-table-column prop="task" label="Task" width="150">
          <template #default="{ row }">
            <el-tag :type="getTaskTag(row.task)">{{ row.task.toUpperCase() }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="cron" label="Schedule (Cron)" />
        <el-table-column label="Actions" width="120" fixed="right">
          <template #default="{ row }">
            <el-popconfirm title="Delete this schedule?" @confirm="handleDelete(row.id)">
              <template #reference>
      ...
      <el-empty v-if="schedules.length === 0 && !loading" description="No schedules configured" />
    </el-card>

    <!-- Create Schedule Dialog -->
    <el-dialog v-model="showCreateDialog" title="Create Automated Task" width="500px">
      <el-form :model="form" label-width="120px">
        <el-form-item label="Target Server">
          <el-select v-model="form.server_id" placeholder="Select Server" style="width: 100%">
            <el-option 
              v-for="s in servers" 
              :key="s.id" 
              :label="s.name" 
        ...
        <el-form-item label="Task Type">
          <el-select v-model="form.task" placeholder="Select Task" style="width: 100%">
        ...
        <el-form-item label="Schedule (Cron)">
          <el-input v-model="form.cron" placeholder="0 0 * * * (Every midnight)" />
          <div class="helper-text">Format: Minute Hour Day Month DayOfWeek</div>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showCreateDialog = false">Cancel</el-button>
        <el-button type="primary" @click="handleCreate" :loading="actionLoading">Create</el-button>
      </template>
    </el-dialog>
  </div>
</template>
